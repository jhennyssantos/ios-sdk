...
public struct WebSocketEvents {
    ...
    public var close : (_ code: Int, _ reason: String, _ wasClean: Bool) -> Void = {(code, reason, wasClean) in}
    /// An event to be called when an error occurs.
    public var error : (_ error: Error) -> Void = {(error) in}
    /// An event to be called when a message is received from the server.
    public var message : (_ data: Any) -> Void = {(data) in}
    /// An event to be called when a pong is received from the server.
    public var pong : (_ data: Any) -> Void = {(data) in}
    ...
    public var end : (_ code: Int, _ reason: String, _ wasClean: Bool, _ error: Error?) -> Void = {(code, reason, wasClean, error) in}
}
...
private class InnerWebSocket: Hashable {
    ...
    func step() {
        ...
        do {
            ...
            if stage == .openConn || stage == .readResponse {
                ...
                if frame == nil {
                    frame = Frame.makeClose(1006, reason: "Abnormal Closure")
                    self.closeCode = frame?.statusCode ?? 0
    ...
    @inline(__always) func fire(_ block: () -> Void) {
...
open class WebSocket: NSObject {
    ...
    public convenience init(_ url: String, subProtocols: [String], observerTimeout: WebsocketTimeoutProtocol) {
